...
import { CriticalityIndicator } from "./industrial/CriticalityIndicator";
...
export function AssetIntelligencePanel({ activeStep }: AssetIntelligencePanelProps) {
  ...
  // Not sticky, and no longer an <aside>: this is one card of three inside the
  // form's sidebar column, which owns that landmark. Pinning just this card
  // left the two below it sliding underneath. The max-height and internal
  // scroll below still keep it inside the viewport on their own.
  return (
    <div className={cn(
      ...
      <PanelSection title="Machine Type">
        ...
        {/* The line above names the level; this spells out what that level means
            for reliability. It renders nothing for a level with no mapping, so
            it is safe to leave mounted. */}
        <CriticalityIndicator level={data.machine_criticality ?? ""} />
      </PanelSection>
